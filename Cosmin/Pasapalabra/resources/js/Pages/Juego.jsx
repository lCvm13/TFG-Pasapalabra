...
export default function Juego({ preguntas, pasapalabra, partida, auth, preguntas_partida }) {
  ...
  useEffect(() => {
    ...
    preguntas_partida.forEach(element => {
      if (progresoActual === preguntas.length - 1) {
        console.log(partida.num_aciertos)
        console.log(partida.num_sin_contestar)
        console.log(partida.num_fallados)
        console.log(partida.num_aciertos + partida.num_fallados == partida.sin_contestar)
        if (preguntas_partida.num_aciertos + preguntas_partida.num_fallados == preguntas_partida.sin_contestar) {
          ...
          return location.href = route('partida.index')
        } else {
          indice = preguntas_partida.findIndex(obj => obj.respuesta_usuario === "")
        }
      } else {
  ...
  //   console.log(preguntas_partida)
  //   let checkPasap = preguntas_partida.find(e => e.id_pregunta == preguntas[progreso].id)
  //   if (checkPasap) {
  //     router.patch((`/preguntas_partidas/${partida.id}`), { respuesta_usuario: respuestaUsuario, id_pregunta: preguntas[progreso].id })
  //   } else {
  ...
  //     } else {
  //       console.log(preguntas_partida)
  //       const indice = preguntas_partida.findIndex(obj => obj.respuesta_usuario === "");
  //       console.log(indice)
  //       console.log(progreso)
  //       setProgreso(indice - 1)
  //       setRespuestaUsuario('');
  //       return;
  //     }
  //   }
  ...
  const handleSubmit = (event) => {
    ...
    if (respuestaUsuario.trim() == "") {
      ...
      document.getElementById(preguntas[progreso].letra.toLowerCase()).style.backgroundColor = "green";
      // router.patch((`/partida/${partida.id}`), { sin_contestar: partida.sin_contestar - 1, num_aciertos: partida.num_aciertos + 1 })
      // datos.concat({ sin_contestar: partida.sin_contestar - 1, num_aciertos: partida.num_aciertos + 1 })
      datos['sin_contestar'] = partida.num_sin_contestar - 1
      datos['num_aciertos'] = partida.num_aciertos + 1
    } else {
      document.getElementById(preguntas[progreso].letra.toLowerCase()).style.backgroundColor = "red";
      console.log()
      // router.patch((`/partida/${partida.id}`), { sin_contestar: partida.sin_contestar - 1, num_fallados: partida.num_fallados + 1 })
      // datos.concat({ sin_contestar: partida.sin_contestar - 1, num_fallados: partida.num_fallados + 1 })
      datos['sin_contestar'] = partida.num_sin_contestar - 1
      datos['num_fallados'] = partida.num_fallados + 1
    }
    let checkPasap = preguntas_partida.find(e => e.id_pregunta == preguntas[progreso].id)
    if (checkPasap) {
      router.patch((`/preguntas_partidas/${partida.id}`), { respuesta_usuario: respuestaUsuario, id_pregunta: preguntas[progreso].id })
      ...
      //   id_usuario: auth.user.id
      // })
      datos['respuesta_usuario'] = respuestaUsuario
      datos['id_partida'] = partida.id
      datos['id_pregunta'] = preguntas[progreso].id
    ...
    router.post(route('partida.combined'), datos);
